...
(function () {
    ...
    function Walt(source, destination, ignore) {
        this.VERSION = '0.1.2'; // TODO: Read version from package.json?
            ...
            if (_.find(manifest.fileTypes, function (fileType) {
                ...
                if (self.pluginConfig(manifest.name).enabled === true) {
                    try {
                        ...
                        self.emit('error', {
                            msg: 'Error while instantiating plugin %s: %s',
                            args: [manifest.toString(manifest), e.toString()]
                        });
                    }
                }
            }
     ...
     * @private
     */
    Walt.prototype.logline = function () {
